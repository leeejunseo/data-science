...
-   **세부 계획**:
    -   **오류 수정 및 안정화**: 다양한 발사 조건에서도 시뮬레이션이 중단되지 않도록 안정성을 높입니다.
        ...
        -   **연료 소모에 따른 동역학 개선**: 현재 시뮬레이터는 연료 소모에 따른 질량 변화를 반영하고 있으나, 연료가 소모됨에 따라 변화하는 무게중심(Center of Gravity)과 이에 따른 관성 모멘트의 동적 변화는 충분히 반영하지 못하고 있습니다. 향후 연료 탱크의 형상과 위치를 고려한 시간에 따른 무게중심 이동 모델을 구현하고, 이를 통해 비행 중 자세 안정성과 제어 특성의 변화를 더욱 정밀하게 시뮬레이션할 계획입니다.
