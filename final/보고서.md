...
9. [미사일 판별을 위한 Signature 물리량 발견](#9-미사일-판별을-위한-signature-물리량-발견)
10. [시그니처 분석 방법 선택](#10-시그니처-분석-방법-선택-정연우-김찬진)
11. [결론 및 향후 과제](#11-결론-및-향후-과제)
12. [팀원 기여도](#12-팀원-기여도)
13. [오픈소스 참조 및 활용](#13-오픈소스-참조-및-활용)
14. [학술 문헌 정리](#14-학술-문헌-정리-apa-style)
15. [감사의 글](#15-감사의-글)
...
### 9.4 (시행착오 정리) FFT 기반 6DOF 물리량의 위치와, 최종 분류 입력의 범위
...
### 9.5 레이더 관측 기반 15개 시그니처 설계 (김찬진)

#### 9.5.1 설계 원칙: "레이더 관측 가능성"으로 변인 통제
...
| **해석 가능성(Interpretability)** | 분류 결과를 "비행 궤적의 크기/형상/감속 특성/기동성"으로 설명 가능 |
| **모델 일반화(Generalization)** | 시뮬레이터 내부 파라미터(질량, 추력, 제어이득 등)에 과적합하지 않도록 입력에서 배제 |

> **문헌 근거:** 레이더 기반 분류 연구에서도 레이더가 제공하는 직접/파생 파라미터(위치, 속도, 가속도, jerk, specific energy, 고도 등)를 입력으로 사용해 표적을 분류하는 흐름이 일반적이다 (Singh et al., 2014; JHU APL Tech Digest).

#### 9.5.2 최종 사용 시그니처(15개) 목록과 의미

아래 15개는 **"레이더 트랙으로부터 계산 가능"**하도록 구성하였다.
(직접 관측: 거리/방위/고각 → 3D 위치, 도플러/차분 → 속도, 추가 차분/필터링 → 가속도·곡률·에너지 등)

| No | Signature (Feature) | 단위 | 의미 (레이더 트랙 기반 해석) |
|---:|---------------------|------|------------------------------|
| 1 | max_altitude_km | km | 최대 고도(아포지) |
| 2 | final_range_km | km | 최종 사거리(다운레인지) |
| 3 | impact_angle_deg | deg | 종말 단계 낙하각(말단 속도벡터의 하강 각도) |
...
| 13 | alpha_std_deg | deg | 등가 받음각(또는 기동각) 변동성의 표준편차 |
| 14 | q_max_deg_s | deg/s | 등가 피치율(비행경로각 변화율 등으로 근사) 최대값 |
| 15 | alpha_q_correlation | - | 등가 받음각–피치율 상관(기동 패턴의 결합 특성) |

> **참고:** 13~15의 "alpha/q"는 시뮬레이터 내부 공력각 그대로를 쓰겠다는 의미가 아니라, 레이더 트랙(3D 속도/가속도)로부터 **기동성/자세변화의 '등가 지표'**를 구성해 사용한다는 뜻이다. (예: 비행경로각/곡률/수직가속 성분 등을 각도로 환산, 시간미분으로 변화율 추정)

#### 9.5.3 왜 이 15개 시그니처인가? (선정 근거)
...
## 10. 시그니처 분석 방법 선택 (정연우, 김찬진)

본 프로젝트에서는 시그니처 후보를 크게 두 계열로 검토했다.

### 10.1 후보 A: FFT 기반(동특성) 시그니처
...
### 10.2 후보 B: 레이더 트랙 기반(관측 가능) 시그니처 — 최종 채택
...
### 13.2 JSBSim (NASA/Lockheed Martin 기원)
