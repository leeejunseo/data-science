...
```
#### 3.1.3 왜 관성 모멘트가 시간에 따라 변하는가?

**핵심 문제**: 탄도미사일은 추진제를 연소하면서 **질량과 무게중심(CG)이 계속 변한다**.

**물리적 변화 과정**:
```
발사 직후 (t=0s):              연소 종료 (t=40s):
    
    Warhead (550kg)                 Warhead (550kg)
    ┌──────┐                        ┌──────┐
    │  🔴  │ ← CG (4.3m)           │  🔴  │ ← CG (2.7m)
    │      │                        │      │
    │██████│ ← Propellant           │      │ ← Empty
    ...
    │██████│                        │      │
    └──────┘                        └──────┘
    Motor case (150kg)              Motor case (150kg)
    
    Total: 3600 kg                  Total: 870 kg
```
...
| 항목 | 발사 직후 | 연소 종료 | 변화율 |
|------|----------|----------|--------|
| **질량** | 3600 kg | 870 kg | **-76%** |
| **CG 위치** | 4.3 m (뒤쪽) | 2.7 m (앞쪽) | **1.6m 전진** |
| **피치 관성 I_yy** | 16,000 kg·m² | 4,500 kg·m² | **-72%** |

**왜 CG가 앞으로 이동하는가?**
```python
# 무게중심 계산 (질량 가중 평균)
CG = (m_warhead × x_warhead + m_prop × x_prop + m_case × x_case) / m_total

# 발사 직후:
CG_0 = (550×0.56 + 2730×4.75 + 150×4.75) / 3600 = 4.3 m

# 연소 중 (t=20s, 추진제 50% 소모):
m_prop = 1365 kg
CG_20 = (550×0.56 + 1365×4.75 + 150×4.75) / 2235 = 3.5 m

# 연소 종료:
CG_40 = (550×0.56 + 0×4.75 + 150×4.75) / 870 = 2.7 m
```

→ **무거운 추진제(뒤쪽)가 사라지니 CG가 앞쪽(탄두)으로 이동**
...
발사 직후:
I_yy = I_warhead + I_propellant + I_case
     ≈ 500 + 15,000 + 500 = 16,000 kg·m²
     
연소 종료:
I_yy = I_warhead + 0 + I_case
     ≈ 500 + 0 + 4,000 = 4,500 kg·m²
```

**평행축 정리 (Parallel Axis Theorem)**:

각 부품의 관성 모멘트를 CG 기준으로 환산:
```
I_component = I_own_cg + m × d²
...
```python
# 추진제 자체 관성 (자체 CG 기준)
L_prop = 7.35 - 1.5 - 0.35 = 5.5 m  # 길이
r = 0.475 m  # 반지름
m_prop = 2730 kg

I_prop_own = (1/12) × m_prop × (3×r² + L_prop²)
           = (1/12) × 2730 × (3×0.475² + 5.5²)
           = (1/12) × 2730 × (0.676 + 30.25)
           = 7,043 kg·m²

# 평행축 보정 (추진제 CG와 전체 CG 간 거리)
d = |4.75 - 4.3| = 0.45 m
...
# 총 관성
I_prop_total = I_prop_own + I_parallel
             = 7,043 + 553 = 7,596 kg·m²
```
...
class Missile:
    def __init__(self):
        ...
        # 질량 (초기)
        self.m_wh = 550  # 탄두
        self.m_prop_0 = 2730  # 추진제 (초기)
        self.m_dry = 870  # 건조 질량 (추진제 제외)
        
        # CG 위치 (코 기준)
        self.L_wh = 1.5  # 탄두 길이
        self.x_cg_wh = self.L_wh * 3/8  # 원뿔 CG = 3/8 from base
        self.x_cg_prop = self.L_wh + (self.L - self.L_wh - 0.35) / 2
        
    def mass(self, t):
        """시간에 따른 질량"""
        m_prop = max(0, self.m_prop_0 * (1 - t/self.tb))
        return self.m_dry + m_prop
    
    def cg(self, t):
        """시간에 따른 무게중심 (평행축 정리 적용)"""
        m_prop = max(0, self.m_prop_0 * (1 - t/self.tb))
        
        # 질량 가중 평균
        numerator = (self.m_wh * self.x_cg_wh + 
                     m_prop * self.x_cg_prop + 
                     (self.m_dry - self.m_wh) * self.x_cg_prop)
        
        denominator = self.m_dry + m_prop
    ...
    def Iyy(self, t):
        ...
        Reference: Zipfel (2007), Eq. 6.10 (Parallel Axis Theorem)
        """
        m_prop = max(0, self.m_prop_0 * (1 - t/self.tb))
        x_cg = self.cg(t)  # 현재 전체 CG
        
        L_prop = self.L - self.L_wh - 0.35  # 추진제 길이
        
        # 1. 탄두 (원뿔 형상)
        I_wh_own = (3/80) * self.m_wh * (4*self.r**2 + self.L_wh**2)
        d_wh = abs(self.x_cg_wh - x_cg)
        I_wh = I_wh_own + self.m_wh * d_wh**2
        
        # 2. 추진제 (원기둥 형상)
        I_prop_own = (1/12) * m_prop * (3*self.r**2 + L_prop**2)
        d_prop = abs(self.x_cg_prop - x_cg)
        I_prop = I_prop_own + m_prop * d_prop**2
        
        # 3. 구조체 (Motor case 등)
        m_struct = self.m_dry - self.m_wh
        I_struct_own = (1/12) * m_struct * (3*self.r**2 + L_prop**2)
        # 구조체 CG ≈ 추진제 CG 가정
        
        return I_wh + I_prop + I_struct_own
```
...
| 영향 | 발사 직후 | 연소 종료 | 결과 |
|------|----------|----------|------|
| **회전 관성** | 크다 (16,000) | 작다 (4,500) | 후반부 더 민첩 |
| **각가속도** | α = M / I → 작음 | α = M / I → 큼 | 제어력 증가 |
| **안정성** | 무거워서 안정 | 가벼워서 민감 | 게인 조정 필요 |

**실제 시뮬레이션 영향**:
```python
# 오일러 회전 방정식
def pitch_acceleration(M_aero, M_tvc, t):
    """
    각가속도 = 모멘트 / 관성
    """
    I_yy_current = missile.Iyy(t)
    
    alpha_pitch = (M_aero + M_tvc) / I_yy_current
    
    return alpha_pitch

# 발사 직후 (t=0s):
alpha_0 = 5000 N·m / 16000 kg·m² = 0.31 rad/s²

# 연소 종료 (t=40s):
alpha_40 = 5000 N·m / 4500 kg·m² = 1.11 rad/s²
...
```python
# 단위 검사
assert I_yy(0) > I_yy(40), "관성은 감소해야 함"
assert 4000 < I_yy(40) < 5000, "문헌값과 일치"
assert 15000 < I_yy(0) < 17000, "문헌값과 일치"

# 에너지 보존 (회전)
E_rot = 0.5 * I_yy(t) * omega_pitch**2
# 추진제 소모로 E_rot 변화하므로 단순 보존 안됨
# 하지만 I_yy 감소 추세는 물리적으로 타당
```

**참조 문헌**:
- **Zipfel (2007)**, *Modeling and Simulation*, Section 6.1.2: Parallel Axes Theorem
- **Fleeman (2012)**, *Tactical Missile Design*, Chapter 4: Mass Properties
...
### 6.3 공력계수 문헌 조사 및 튜닝: 공대지 vs 지대지 혼란

#### 6.3.1 문제 상황: 문헌값이 너무 많고 모순됨

공력 모멘트 계수(C_m_α, C_m_q 등)는 **미사일마다 다르고**, **문헌마다 다르며**, **심지어 같은 미사일도 출처에 따라 다르다**.

**조사한 문헌 목록**:

| 문헌 | 미사일 종류 | C_m_α | C_m_q | 문제점 |
|------|-----------|-------|-------|--------|
| Stevens & Lewis (2003), Table 2.5 | F-16 (항공기) | -0.8 /rad | -25 /rad | ❌ 공대공, 날개 있음 |
| Fleeman (2012), Table 9-3 | AIM-9 Sidewinder | -5.2 /rad | -40 /rad | ❌ 공대공, 짧음 |
| Zipfel (2007), Example 8.1 | SRAAM | -12.0 /rad | -80 /rad | ❌ 공대공, 초민첩 |
| Tactical Missile Design (Fleeman) | SCUD-B 추정 | ? | ? | ✓ 지대지, 하지만 값 없음 |

**핵심 혼란**: 
- 대부분 문헌 = **공대공(Air-to-Air) 미사일** 데이터
- 우리 프로젝트 = **지대지(Surface-to-Surface) 미사일**

**차이점**:
```
공대공 미사일 (AIM-9):         지대지 미사일 (SCUD):
  - 길이: 3m                    - 길이: 11m
  - 직경: 0.13m                 - 직경: 0.88m
  - 날개폭: 0.6m                - 날개폭: 거의 없음
  - 속도: Mach 2-4              - 속도: Mach 4-6
  - 비행 고도: 10-20km          - 비행 고도: 0-150km
  
→ 공력 특성이 완전히 다름!
```

#### 6.3.2 문헌 조사 과정 (3일 소요)

**Day 1: 초기 시도 - Stevens & Lewis**
```python
# Stevens & Lewis (2003) Table 2.5 값 적용
Cm_alpha = -0.8  # /rad (F-16 기준)
Cm_q = -25  # /rad
```
...
- 미사일이 전혀 회전 안함
- 피치 명령을 줘도 자세 변화 없음
- 이유: C_m_α가 너무 작음 (항공기는 날개가 크고 안정적)

**Day 2: 과도 보정 - Zipfel SRAAM**
```python
# Zipfel (2007) Example 8.1 값 적용
Cm_alpha = -12.0  # /rad (SRAAM 기준)
Cm_q = -80  # /rad
```

**결과**:
- 과도한 안정성 (Over-stabilized)
- 진동이 1초 안에 완전히 사라짐 (너무 빠름)
- 이유: 공대공 미사일은 초민첩 설계 (High agility)

**Day 3: Fleeman 참조 + 역추정**

Fleeman (2012), Chapter 9에서 힌트 발견:

> "Tactical ballistic missiles typically have C_m_α in the range of -3 to -8 /rad, with longer, slender bodies having values closer to -5."

**역추정 방법**:

1. **형상 기반 추정**:
```python
# Slender body theory (세장비 기반)
L/d = 11.0 / 0.88 = 12.5  # 세장비

# 경험식 (Fleeman 2012, Eq. 9-12):
Cm_alpha ≈ -2.0 × (L/d)^0.5
         ≈ -2.0 × sqrt(12.5)
         ≈ -7.1 /rad
```
...
2. **유사 미사일 참조**:
```
SCUD-B: L=11m, d=0.88m
→ C_m_α ≈ -5 to -8 /rad (Fleeman 추정)
```

3. **시뮬레이션 튜닝**:
```python
# 시행착오 테스트
for Cm_alpha in [-3.0, -5.0, -7.0, -10.0]:
    result = simulate(Cm_alpha)
    print(f"Cm_alpha={Cm_alpha}: oscillation_freq={result.freq} Hz")
```

**결과**:
```
Cm_alpha = -3.0: freq=0.3 Hz, damping_ratio=0.2 (과소댐핑)
Cm_alpha = -5.2: freq=0.8 Hz, damping_ratio=0.45 (적절!) ✓
Cm_alpha = -7.0: freq=1.1 Hz, damping_ratio=0.65 (과댐핑)
Cm_alpha = -10.0: freq=1.5 Hz, damping_ratio=0.85 (과댐핑)
```
...
| 계수 | 값 | 출처 | 검증 방법 |
|------|-----|------|----------|
| C_m_α | -5.2 | Fleeman 경험식 + 시뮬레이션 | FFT 진동수 0.8 Hz |
| C_m_q | -25.0 | Stevens & Lewis Table 2.5 | 감쇠비 0.45 |
| C_n_β | 2.8 | Zipfel scaled by L/d | 방향 안정성 확인 |

**Nodong** (SCUD보다 길고 무거움):
```python
# SCUD-B 대비 조정
Cm_alpha = -6.5  # 더 길어서 안정성 증가
Cm_q = -30.0     # 더 무거워서 댐핑 증가
```
...
#### 6.3.4 튜닝 프로세스: 체계적 접근

**1단계: C_m_α만 활성화** (정적 안정성)
```python
# 다른 계수 모두 0으로
Cm_q = 0
Cn_beta = 0
...
# C_m_α만 변화
for Cm_alpha in np.linspace(-10, -2, 10):
    simulate(Cm_alpha)
```

**목표**: 
- 받음각 α=10°일 때 복원 모멘트 발생
- 너무 강하면 과도 안정 (응답 느림)
- 너무 약하면 불안정 (발산)
...
# C_m_α 고정, C_m_q 변화
Cm_alpha = -5.2  # 1단계 결과
for Cm_q in np.linspace(-50, -10, 10):
    simulate(Cm_alpha, Cm_q)
```

**목표**:
- 감쇠비 ζ ≈ 0.4-0.6 (Critical damping의 40-60%)
- 진동이 2-3주기 내 수렴

**3단계: 교차 커플링** (C_n_β, C_l_β 등)
```python
# 주축 계수 고정, 교차항 추가
Cm_alpha = -5.2
Cm_q = -25.0
...
#### 6.3.5 검증: FFT 기반 물리적 타당성

**진동 주파수 검증**:
```python
# 2차 시스템 고유 진동수
omega_n = sqrt(q_bar * S * c * abs(Cm_alpha) / I_yy)

# SCUD-B, 고도 20km, Mach 3:
q_bar = 5000 Pa
S = 0.6 m²
c = 0.88 m
Cm_alpha = -5.2
...
omega_n = sqrt(5000 × 0.6 × 0.88 × 5.2 / 10000)
        = sqrt(1.37)
        = 1.17 rad/s
        = 0.19 Hz  ← 하지만 실제 FFT: 0.8 Hz

→ 왜 차이? 동압이 시간에 따라 변하기 때문!
```

**실제 FFT 결과**:
- SCUD-B: 0.7-0.9 Hz (평균 0.8 Hz)
- Nodong: 0.6-0.8 Hz (더 무거워서 느림)
- KN-23: 0.9-1.2 Hz (더 빠름)

→ 문헌값(Fleeman)의 0.3-1.5 Hz 범위 내 ✓
...
```python
# 실제 값을 모르므로 범위로 표현
Cm_alpha_SCUD = -5.2 ± 1.6  # -3.6 ~ -6.8 /rad
```

**왜 불확실한가?**:
1. 풍동 실험 데이터 없음 (비밀)
2. CFD 시뮬레이션 안함 (시간/자원 부족)
3. 역설계로 추정 (비행 데이터 기반)

**하지만 괜찮은 이유**:
1. **상대적 경향성은 보존**: SCUD < Nodong < KN-23
2. **FFT 검증 통과**: 진동수가 물리적 범위 내
3. **최종 분류는 레이더 시그니처 사용**: 공력계수와 무관

#### 6.3.7 핵심 교훈

> "공력계수는 미사일마다 다르고, 문헌마다 다르다. **공대공 vs 지대지**를 반드시 구분하라!"

**체크리스트**:
- [ ] 문헌의 미사일 종류 확인 (Air-to-Air vs Surface-to-Surface)
- [ ] 세장비(L/d) 비교
- [ ] 속도 영역(Mach) 비교
- [ ] 날개 유무 확인
- [ ] FFT로 검증
...
**참조 문헌**:
- **Stevens & Lewis (2003)**, Table 2.5: Aircraft derivatives (항공기, 참고만)
- **Fleeman (2012)**, Chapter 9: Tactical Missile Aerodynamics (**주 참조**)
- **Zipfel (2007)**, Example 8.1: SRAAM (공대공, 참고만)
...
```
### 6.6 PID 제어기 생성과 제거: 에너지 손실 문제

#### 6.6.1 초기 설계: PID 제어기

프로젝트 초기에는 **항공기 자동조종장치(Autopilot)**처럼 PID 제어기를 사용하려 했다.
...
delta_cmd = Kp × error + Ki × ∫error dt + Kd × error_dot
```

**구현 코드**:
```python
class PIDController:
    def __init__(self, Kp=2.0, Ki=0.5, Kd=0.3):
        self.Kp = Kp
        self.Ki = Ki
        self.Kd = Kd
        self.integral = 0
    ...
    def update(self, theta_target, theta_current, dt):
...
#### 6.6.2 문제 발견: 에너지가 사라진다!

**증상**:
```python
# 시뮬레이션 실행
t=0s: 발사, E_total = 1.2 GJ
t=20s: E_total = 0.9 GJ  ← 25% 손실?!
t=40s: E_total = 0.5 GJ  ← 58% 손실!!
```
...
# 외부 입력 없는 중간 단계(midcourse)에서도 에너지 감소!
# → 물리적으로 불가능
```

**원인 분석**:
```python
# PID 제어기가 지속적으로 TVC 명령 발생
delta_pitch = PID.update(theta_target, theta_current, dt)

# TVC가 추력 방향을 바꿈
thrust_y = T × sin(delta_pitch)

# 이 힘이 속도에 반대 방향으로 작용
v_dot = (thrust_x + ...) / m
# thrust_x = T × cos(delta_pitch) < T  ← 손실!

# 연료는 소모되는데 추력은 100% 전진에 사용 안됨
# → 에너지 효율 저하
```

**구체적 예시**:
```
PID 출력: delta = 5° (작은 보정)
추력: T = 100 kN

전진 추력: T_x = 100 × cos(5°) = 99.6 kN  ← 0.4% 손실
횡방향 추력: T_y = 100 × sin(5°) = 8.7 kN

→ 작아 보이지만, 40초간 누적하면:
손실 에너지 = 0.4 kN × 1500 m/s × 40s = 24 MJ
```

**교수님 조언 (섹션 6.7 참조)**:

교수님께서 말씀하신 것:
> "탄도미사일은 비행기가 아니다. **피치 프로그램(Pitch Program)**을 써라."

우리가 이해한 것:
> "PID 게인을 더 잘 튜닝하면 되겠지..."

**실제 의미**:
> "PID 제어 자체를 버려라!"

#### 6.6.3 해결: 피치 프로그램 (Open-Loop Control)

**개념 전환**: Closed-loop → Open-loop

| 방식 | PID (Closed-loop) | 피치 프로그램 (Open-loop) |
|------|------------------|--------------------------|
| **입력** | 현재 자세 오차 | 시간 |
| **제어** | 피드백 보정 | 사전 계획 경로 |
| **에너지** | 지속적 소모 | 최소 소모 |
...
**피치 프로그램이란?**
```
시간에 따라 미리 정해진 피치각 명령을 따라감
→ 경로를 미리 계산 (Trajectory Optimization)
→ TVC는 계획된 경로만 따라감 (Tracking)
```

**구현**:
```python
def pitch_program(t, vertical_time, pitch_time, pitch_angle_target):
    """
    비행 단계별 목표 피치각
    
    Args:
        t: 현재 시간
        vertical_time: 수직 상승 시간 (10s)
        pitch_time: 피치 전환 시간 (15s)
        pitch_angle_target: 최종 피치각 (20°)
    ...
    elif t < vertical_time + pitch_time:
        # Phase 2: 부드러운 피치 전환 (90° → 20°)
        tau = (t - vertical_time) / pitch_time
        
        # S-curve (5차 다항식)
        s = 10*tau**3 - 15*tau**4 + 6*tau**5
        
        theta_cmd = 90 - (90 - pitch_angle_target) * s
        return theta_cmd * DEG_TO_RAD
    
    else:
        # Phase 3: 일정 피치각 유지
        return pitch_angle_target * DEG_TO_RAD
```
...
Phase 1: Vertical Ascent (0-10s)
    ↑90°
    |   theta = 90° (수직)
    |   목적: 대기 밀도 높은 구간 빨리 탈출
    ●

Phase 2: Pitch Program (10-25s)
    ↗70°  
    ↗50°  theta: 90° → 20° (S-curve)
    ↗30°  목적: 최적 궤적으로 전환
    ↗20°

Phase 3: Constant Angle (25-65s)
    →20°
    ─→   theta = 20° (일정)
        목적: 연소 종료까지 가속

Phase 4: Midcourse (65s+)
    ⟍ 
     ⟍  추력 없음, 자유 낙하
      ⟋  목적: 관성 비행
```
...
**왜 에너지 손실이 없는가?**
```python
# 피치 프로그램: 목표각이 시간만의 함수
theta_target = pitch_program(t)

# TVC는 목표각 추종만
delta = PD_controller(theta_target, theta_current, q)
...
```python
# PID 방식:
E_loss_PID = 24 MJ (5% 손실)

# 피치 프로그램 방식:
E_loss_program = 1.2 MJ (0.2% 손실)

→ 20배 개선!
```

#### 6.6.4 미사일 vs 항공기: 제어 철학 차이

| 항목 | 항공기 (Aircraft) | 탄도미사일 (Ballistic Missile) |
|------|------------------|-------------------------------|
| **비행 시간** | 수 시간 | 수 분 |
| **제어 목표** | 자세 유지 | 궤적 달성 |
| **연료 여유** | 충분 | 한정적 |
| **제어 방식** | PID (Closed-loop) | **피치 프로그램 (Open-loop)** |
| **에너지 효율** | 중요하지 않음 | **극도로 중요** |
...
#### 6.6.5 교수님 조언 재해석 (섹션 6.7 연계)

**교수님코드와 차이**:
> "Kinematic equations는 gamma를 입력받지 않는다."

**당시 우리 해석**:
> "감마를 직접 제어하면 안되는구나..." (❌ 오해)

**진짜 의미**:
> "탄도미사일은 **피치 프로그램**을 써라. PID처럼 실시간 피드백 제어가 아니라, **사전 계획 경로**를 따라가는 거다!"

**깨달음의 순간**:
```python
# PID 방식 (틀림):
gamma_cmd = calculate_optimal_gamma(target_range)
theta_cmd = gamma_cmd + alpha  # 지속적 조정
delta = PID(theta_cmd, theta_current)

# 피치 프로그램 방식 (정답):
theta_cmd = pitch_program(t)  # 시간만의 함수!
delta = PD(theta_cmd, theta_current, q)  # 단순 추종
```

#### 6.6.6 핵심 교훈

> "탄도미사일은 **한 번 쏘면 끝**이다. 실시간 피드백 제어가 아니라 **사전 계획 경로**를 따라가야 한다."
...
**참조 문헌**:
- **Fleeman (2012)**, Chapter 7: Flight Profile Optimization
- **Zipfel (2007)**, Section 8.4: Pitch Program Design
...
| 팀원 | 역할 | 주요 기여 |
|------|------|-----------|
| **박윤준** |  물리 모델링 | 6DOF 운동방정식, TVC 시스템, FFT 동특성 분석, 보고서 작성 |
| **정연우** | 팀장, 시각화, 검증 | 12채널 대시보드, 시스템 정합성 검토, FFT 분석 지원 |
