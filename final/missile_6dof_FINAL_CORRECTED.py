...
class Missile6DOF_Authentic:
    ...
    def dynamics(self, t, state):
        ...
        # === 비행 프로그램 (교수님 방식 정확 복제) ===
        phase = self.get_flight_phase(t)
        
        # 프로그램 각속도 (교수님 main.py Line 705)
        if phase == "Vertical":
            dtheta_program = 0  # theta = 90° 유지
        elif phase == "Pitch":
            # theta를 (90° - pitch_angle)로 감소
            theta_target = np.pi/2 - self.pitch_angle_deg * cfg.DEG_TO_RAD
            if theta > theta_target + 0.01:  # 여유
                # dgamma = -pitch_rad / pitch_time (교수님)
                dtheta_program = -self.pitch_angle_deg * cfg.DEG_TO_RAD / self.pitch_time
            else:
                dtheta_program = 0
        else:
            dtheta_program = 0
        
        # 공력 모멘트 (Stevens & Lewis 2003 - 댐핑만)
        Cm = self.C_mq * (q * self.length / (2*V)) if V > 0.1 else 0
        Cn = self.C_nr * (r * self.length / (2*V)) if V > 0.1 else 0
        Cl = self.C_lp * (p * self.length / (2*V)) if V > 0.1 else 0
        
        L_aero = q_dyn * self.wing_area * self.diameter * Cl
        M_aero = q_dyn * self.wing_area * self.length * Cm  # 댐핑만!
        N_aero = q_dyn * self.wing_area * self.length * Cn
        
        # === 회전 운동 방정식 (Euler's equation) ===
        dp = (L_aero + (self.I_yy - self.I_zz) * q * r) / self.I_xx
...
if __name__ == "__main__":
    ...
    for m_type in missiles:
        ...
        try:
            ...
            result = missile.simulate(elevation_deg=10, azimuth_deg=90, 
